...
1. Caricamento di un dataframe delle tracce di riferimento (ad esempio il file legacy `manos_CL7_pixel.csv`) che contiene le coordinate sia in lat/lon sia in pixel, e generazione degli offset di tile con `calc_tile_offsets` (in funzione dello stride).
...
6. Creazione dataset di un anno intero: usando il metodo `get_data_ready_full_year` e `create_master_df_year`, che non limita il caricamento ai soli intervalli temporali attorno ai cicloni contenuti nel dataframe delle tracce di riferimento.
