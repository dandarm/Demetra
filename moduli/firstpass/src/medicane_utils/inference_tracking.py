...
def _compute_sample_record(path: str, pred_xy: torch.Tensor, target_xy: torch.Tensor) -> Dict[str, Optional[Union[float, str]]]:
...
@torch.no_grad()
def inference_epoch(model, data_loader, device) -> tuple[Dict[str, float], List[Dict[str, Optional[Union[float, str]]]]]:
    criterion = torch.nn.MSELoss()
    metric_logger = utils.MetricLogger(delimiter="  ")
    results: List[Dict[str, Optional[Union[float, str]]]] = []
...
def launch_inference_tracking(terminal_args: argparse.Namespace) -> None:
    ...
    if dist.is_available() and dist.is_initialized():
        gathered: List[List[Dict[str, Optional[Union[float, str]]]]] = [None] * dist.get_world_size()
